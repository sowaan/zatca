...
# ZATCA message category -> label colour.
_COLORS = {"error": "#d9534f", "warning": "#e8a33d", "info": "#1f9d55"}


def _section(title, messages, color):
    rows = []
    for message in messages or []:
        ...
        rows.append(f"<li style='margin:2px 0'>{prefix}{text}</li>")
    if not rows:
        return ""
    return (
        f"<div style='margin-top:8px'>"
        f"<div style='font-weight:600;color:{color}'>{title} ({len(rows)})</div>"
...
def format_zatca_response(response):
    ...
    text). Shows the overall status plus any error/warning/info messages, and
    falls back to the raw/pretty text when it is not the expected JSON."""
    ...
    header = (
        f"<div style='font-weight:600;color:{'#1f9d55' if ok else '#d9534f'}'>"
    ...
    lines.append(_section(frappe._("Errors"), validation.get("errorMessages"), _COLORS["error"]))
    lines.append(_section(frappe._("Warnings"), validation.get("warningMessages"), _COLORS["warning"]))
    lines.append(_section(frappe._("Info"), validation.get("infoMessages"), _COLORS["info"]))
