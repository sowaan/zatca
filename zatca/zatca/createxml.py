...
def _is_change_as_allowance(si_doc, tolerance=Decimal("0.05")):
    ...
    - no explicit allowance child rows (allowance_charge)
    - and (paid_amount - grand_total) ≈ allowance_total (within tolerance)
    """
    try:
        paid = Decimal(str(getattr(si_doc, "paid_amount", 0) or 0))
        grand = Decimal(str(getattr(si_doc, "grand_total", 0) or 0))
        # try common allowance fields, fallback zeros
        allow_total_candidates = [
            getattr(si_doc, "allowance_total_amount", None),
            getattr(si_doc, "base_change_amount", None),
            getattr(si_doc, "allowance_total", None),
        ]
        # pick first non-none numeric
        allow_total = None
        for v in allow_total_candidates:
            if v is not None:
                allow_total = Decimal(str(v or 0))
                break
        if allow_total is None:
            allow_total = Decimal("0")

        # if there are explicit allowance child rows, we won't treat as change
        # common child table name: allowance_charge or allowance_charges — check your doctype
        if si_doc.get("allowance_charge") or si_doc.get("allowances") or si_doc.get("allowance_charges"):
            return False

        delta = (paid - grand).quantize(Decimal("0.01"))
        diff = abs(delta - allow_total)
        if delta > 0 and diff <= tolerance:
            return True
    except (InvalidOperation, Exception):
        # on any parsing error, do not attempt to auto-fix
        return False
    return False
>>>>>>> 5928bc3 (Allowance Change Issue)
...
=======
        ...
        # write AllowanceTotalAmount — but avoid writing cash change as allowance
        cbc_AllowanceTotalAmount = ET.SubElement(cac_LegalMonetaryTotal, "cbc:AllowanceTotalAmount")
        cbc_AllowanceTotalAmount.set("currencyID", sales_invoice_doc.currency)

        # Determine allowance total to output:
        # Prefer the explicit allowance_total_amount field if present; fallback to base_change_amount.
        allowance_value = None
        if getattr(sales_invoice_doc, "allowance_total_amount", None) is not None:
            allowance_value = getattr(sales_invoice_doc, "allowance_total_amount")
        elif getattr(sales_invoice_doc, "base_change_amount", None) is not None:
            allowance_value = getattr(sales_invoice_doc, "base_change_amount")
        else:
            allowance_value = 0

        # If this looks like "cash change" (paid - grand == allowance) and there are no allowance lines,
        # treat it as change and output 0.00 for allowance in XML to avoid ZATCA BR-CO-11.
        from decimal import Decimal
        if _is_change_as_allowance(sales_invoice_doc):
            cbc_AllowanceTotalAmount.text = "{:.2f}".format(0)
        else:
            try:
                cbc_AllowanceTotalAmount.text = "{:.2f}".format(Decimal(str(allowance_value or 0)))
            except Exception:
                cbc_AllowanceTotalAmount.text = str(allowance_value or 0)
