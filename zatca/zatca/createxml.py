...
def company_Data(invoice,sales_invoice_doc):
    try:
        ...
        cbc_ID_2.text = custom_company_registration # COmpany CR - Need to have a field in company doctype called company_registration 
...
def delivery_And_PaymentMeans(invoice,sales_invoice_doc, is_return):
    try:
        ...
        if is_return == 1:
            ...
            cbc_InstructionNote.text = "Cancellation or Returned"

        if sales_invoice_doc.is_debit_note == 1 :
            cbc_instruction_note = ET.SubElement(
                cac_PaymentMeans, "cbc:InstructionNote"
            )
...
def delivery_And_PaymentMeans_for_Compliance(invoice,sales_invoice_doc, compliance_type):
    try:
        ...
        if compliance_type in {"3", "4", "5", "6"}:
            cbc_InstructionNote = ET.SubElement(cac_PaymentMeans, "cbc:InstructionNote")
            cbc_InstructionNote.text = "Cancellation or Additional Charge" 
        return invoice
    except (ET.ParseError, AttributeError, ValueError) as e:
        frappe.throw(_(f"Delivery and payment means failed: {e}"))
        return None
