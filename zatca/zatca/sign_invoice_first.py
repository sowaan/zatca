...
def parse_csr_config(csr_config_string):
    """Parse CSR config safely (production-safe)"""
    csr_config = {}

    if not csr_config_string:
        frappe.throw(_("CSR config is empty"))
    ...
    for idx, raw_line in enumerate(lines, start=1):
        ...
        if "=" not in line:
            frappe.throw(
                _(f"Invalid CSR config format at line {idx}: '{raw_line}'")
            )
        ...
        if not key or not value:
            frappe.throw(
                _(f"Invalid CSR config entry at line {idx}: '{raw_line}'")
            )

        csr_config[key] = value
...
def create_csr(zatca_doc, portal_type, company_abbr, csr_config_string=None):
    """
    ...
    try:
        ...
        # Normalize CSR config (single source of truth)
        parsed_csr_config = ""
        ...
        if csr_config_string:
            ...
            if doc.doctype == "Company":
                ...
                company_doc.custom_csr_config = csr_config_string

                company_doc.custom_zatca_invoice_enabled = 1
                company_doc.custom_send_einvoice_background = 1
                company_doc.custom_attach_xml_with_invoice = 1
                company_doc.custom_attach_xml_with_qr_code = 1
                company_doc.custom_attach_qr_code_doctype = 1
                company_doc.custom_attach_e_invoice_send_status_with_invoice = 1
                company_doc.custom_send_invoice_to_zatca = "Live"
                company_doc.custom_pih = "MA=="
                
                company_doc.save(ignore_permissions=True)

            elif doc.doctype == "ZATCA Multiple Setting":
                doc.custom_csr_config = csr_config_string
                doc.save(ignore_permissions=True)

        else:
            frappe.log_error ("no csr config string found", "what to do now")
            # Fallback to persisted config
            if doc.doctype == "Company":
                parsed_csr_config = parse_csr_config(
                    frappe.get_value("Company", {"abbr": company_abbr}, "custom_csr_config")
                )
            else:
                parsed_csr_config = parse_csr_config(doc.custom_csr_config)

        company_csr_data = parsed_csr_config
...
def signxml_modify(company_abbr,finalzatcaxml,source_doc):
    ...
    try:
        ...
        result = extract_certificate_details(company_abbr, source_doc)

        if not result or not isinstance(result, (tuple, list)) or len(result) != 2:
            frappe.throw(_("Invalid certificate details returned"))

        issuer_name, serial_number = result
